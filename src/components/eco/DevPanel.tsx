      ...
      {open && (
        <div className="fixed bottom-36 right-4 z-40 w-80 rounded-2xl border border-white/10 bg-slate-950/95 p-5 shadow-2xl backdrop-blur-2xl md:bottom-24">
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            Simulate profile shifts to verify real-time recalibration.
          </p>
          ...
          <div className="mt-4 space-y-2">
            <Button
              onClick={() => {
                resetTo("low");
                toast.success("Switched to Low-Carbon Champion");
                setOpen(false);
              }}
            ...
            <Button
              onClick={() => {
                resetTo("high");
                toast.success("Switched to Heavy Fossil Fuel");
                setOpen(false);
              }}
            ...
            <Button
              onClick={() => {
                resetTo("default");
                toast.success("Reset to balanced default");
                setOpen(false);
              }}
              variant="outline"
          ...
          <div className="mt-4 rounded-lg border border-white/5 bg-white/[0.02] p-3 text-[11px] text-muted-foreground">
            <div className="font-mono">
              baseline: {state.profile.baselineAnnualTons}t · xp: {state.profile.xp} · credits:{" "}
              {state.profile.credits} · streak: {state.profile.streak}
            </div>
          </div>
        </div>
      )}
