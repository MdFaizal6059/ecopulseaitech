    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg border-emerald-500/20 bg-gradient-to-br from-slate-950 to-emerald-950/40">
        ...
        <p className="text-xs text-muted-foreground">
          PromptWars Challenge 3 · Hackathon submission entries.
        </p>
        <div className="space-y-4 pt-2">
          ...
          <Field
            ...
            label="Live Web Deployment"
            value={deploy}
            onChange={setDeploy}
          />
          <Field
            icon={<Linkedin className="h-4 w-4" />}
            label="LinkedIn Build Journey Post"
            value={post}
            onChange={setPost}
          />
        </div>
        <Button
          onClick={() => {
            toast.success("Submission saved", { description: "Locked in for evaluation." });
            onOpenChange(false);
          }}
