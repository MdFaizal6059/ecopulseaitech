...
export function AIAssistant() {
  ...
  const [log, setLog] = useState<{ role: "user" | "ai"; text: string }[]>([
    { role: "ai", text: "Hi! Tell me what you did today — e.g. \"I ate a vegan salad and took a 5km metro ride\"." },
  ]);
  ...
  const submit = async () => {
    ...
    if (acts.length === 0) {
      setLog((l) => [...l, { role: "ai", text: "I couldn't extract any activities. Try mentioning a distance (e.g. 5km), a meal, or kWh." }]);
    } else {
      acts.forEach(addActivity);
      const total = acts.reduce((s, a) => s + a.co2eKg, 0);
      setLog((l) => [...l, { role: "ai", text: `Logged ${acts.length} activit${acts.length > 1 ? "ies" : "y"} totaling ${total.toFixed(2)} kg CO₂e: ${acts.map(a => a.label).join(", ")}` }]);
  ...
  return (
    <Card className="flex flex-col border-emerald-500/20 bg-gradient-to-br from-slate-900/60 to-emerald-950/20 p-5">
      ...
      <div className="mb-3 max-h-48 flex-1 space-y-2 overflow-y-auto pr-1 text-sm">
        {log.map((m, i) => (
          <div key={i} className={m.role === "ai" ? "text-muted-foreground" : "text-foreground"}>
            <span className={`mr-1.5 text-[10px] uppercase tracking-widest ${m.role === "ai" ? "text-emerald-400" : "text-cyan-400"}`}>
              {m.role === "ai" ? "AI" : "You"}
            </span>
            {m.text}
          </div>
      ...
      >
        <Input value={text} onChange={(e) => setText(e.target.value)} placeholder="Tell me what you did…" className="bg-white/5" />
